...
<div class="container mx-auto px-4 py-8">
    <div class="bg-white rounded-lg shadow-lg p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="bg-gray-50 p-4 rounded-lg">
                    <div class="mb-4">
                        ...
                        <span class="ml-2 px-2 py-1 rounded text-sm {{ $userInfo['status'] === 'active' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                    ...
                    <div>
                        ...
                        <div class="text-sm text-gray-500 mt-1">(осталось {{ $userInfo['days_remaining'] }} дней)</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div>
            <h2 class="text-2xl font-bold mb-6">Ключи подключения</h2>
            <div class="space-y-4">
...
{{--                                    <div class="text-sm text-gray-500">{{ $key['description'] }}</div>--}}
                            ...
                            <div class="flex space-x-2 ml-4">
                                ...
                                <button onclick="showQR('{{ $key['link'] }}')"
                                        class="inline-flex items-center px-3 py-2 border border-gray-300 shadow-sm text-sm leading-4 font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors">
                                    <svg class="h-4 w-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v1m6 11h-6m-6 0h6m6-3v-3m-6 0v-3"/>
                                    </svg>
                                    QR-код
                                </button>
                            </div>
...
<div id="qrModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50">
    <div class="bg-white p-6 rounded-lg max-w-lg w-full mx-4">
        ...
        <div class="flex justify-end space-x-2">
            <button onclick="closeQR()" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-md hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 transition-colors">
                Закрыть
            </button>
        </div>
    </div>
...
<script src="{{ asset('js/vpn-config.js') }}"></script>
@endpush
@endsection
