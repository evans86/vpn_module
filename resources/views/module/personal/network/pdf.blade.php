...
<h2>Профиль</h2>
<table>
    <tr><th>Продавец</th><td>{{ $salesman->name ?? $salesman->username ?? ('ID '.$salesman->id) }}</td></tr>
    <tr><th>Период теста</th>
        <td>{{ $data['period_display']['start'] }} — {{ $data['period_display']['finish'] }}
            ({{ $data['period_display']['tz'] }})
        </td>
    </tr>
    <tr><th>User‑Agent</th><td>{{ $data['env']['ua'] }}</td></tr>
    <tr><th>Таймзона</th><td>{{ $data['env']['tz'] }}</td></tr>
</table>
...
@if(!empty($data['packetLoss']))
    <h2>Пакетные потери (HTTP‑пинги к модулю)</h2>
    <table>
...
@if(!empty($data['doh']))
    ...
    <table>
        ...
        @foreach($data['doh'] as $domain => $res)
            @php
                $g = $res['google'] ?? null; $c = $res['cf'] ?? null;
            @endphp
            <tr>
                <td>{{ $domain }}</td>
...
<table>
    ...
    <tr><th>Значения</th><td class="small">
            {{ implode(', ', array_map(function($x){ return (string)round($x); }, $data['latency']['samples'] ?? [])) }} (мс)
        </td></tr>
</table>
</body>
</html>
