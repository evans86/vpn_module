...
(function (global) {
     ...
     * Узнаваемый знак Telegram (круг со стилизованным самолётиком из открытых glyph-наборов).
     */
    var TG_ICON_PATH =
    ...
    function buildPurchaseBotCardHtml(pb) {
        ...
        var tgIcon =
            '<svg class="h-9 w-9 shrink-0" viewBox="0 0 24 24" aria-hidden="true"><path fill="currentColor" d="' +
            TG_ICON_PATH +
            '"/></svg>';
        var tgIconBtn =
            '<svg class="h-5 w-5 shrink-0 text-white" viewBox="0 0 24 24" aria-hidden="true"><path fill="currentColor" d="' +
            TG_ICON_PATH +
            '"/></svg>';
        return (
            ...
            '<h2 class="text-lg font-bold text-gray-900 md:text-xl">' + heading + '</h2>' +
            (username ? '<p class="mt-0.5 text-sm font-semibold text-gray-700">' + username + '</p>' : '') +
            '</div>' +
            '<div class="flex md:shrink-0">' +
            '<a href="' +
            escapeAttr(pb.url) +
            '" target="_blank" rel="noopener noreferrer" class="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[#229ED9] px-4 py-2.5 text-sm font-semibold text-white shadow transition-colors hover:bg-[#1b8dc4] focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-offset-2 md:w-auto">' +
            tgIconBtn +
            '<span>' +
            cta +
            '</span></a>' +
    ...
    function buildVpnActionToolbarHtml() {
        ...
        return (
            ...
            '<div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3 auto-rows-fr">' +
            ...
            /* Временно: кнопка «Ссылка для Clash»
            '<button type="button" onclick="copyClashSubscriptionUrl()" title="YAML с узлами и правилами DIRECT (?format=clash)" class="' + c + ' border-amber-200 text-amber-900 bg-amber-50 hover:bg-amber-100 focus:ring-amber-500">' +
            '<span class="inline-flex flex-col sm:flex-row items-center justify-center gap-1.5"><svg class="h-5 w-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h10M4 18h16"/></svg><span>Ссылка для Clash</span></span></button>' +
            */
