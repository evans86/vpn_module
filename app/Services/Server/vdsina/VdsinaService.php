...
class VdsinaService
{
    ...
    public function reboot(Server $server): void
    {
        ...
        try {
            $this->vdsinaApi->rebootServer((int) $server->provider_id);
            Log::info('VDSina server reboot requested', [
                'server_id' => $server->id,
                'provider_id' => $server->provider_id,
                'source' => 'server',
            ]);
        } catch (Exception $e) {
            Log::error('VDSina server reboot failed', [
                'server_id' => $server->id,
                'provider_id' => $server->provider_id,
                'error' => $e->getMessage(),
                'source' => 'server',
            ]);
            throw new RuntimeException('Ошибка перезагрузки VDSina: ' . $e->getMessage());
        }
    }
