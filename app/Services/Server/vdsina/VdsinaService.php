...
class VdsinaService
{
    ...
    public function ping(Server $server): bool
    {
        try {
            ...
            // 1. Проверка TCP-порта (основной метод)
            if ($this->checkTcpPort($server->ip, 22, 1)) { // Проверяем SSH порт
                return true;
            }

            // 2. Проверка HTTP (если есть доменное имя)
            if (!empty($server->host) && $this->checkHttp($server->host)) {
                return true;
            }

            // 3. Альтернативная проверка через DNS
            if ($this->checkDnsResolution($server->ip)) {
                return true;
            }
     ...
     * Проверка TCP-порта
     */
    private function checkTcpPort(string $ip, int $port, float $timeout): bool
    {
        try {
     ...
     */
    private function checkHttp(string $host): bool
    {
        try {
            $ch = curl_init();
     ...
     */
    private function checkDnsResolution(string $ip): bool
    {
        try {
            return gethostbyname($ip) === $ip;
        } catch (Exception $e) {
            return false;
        }
    }
