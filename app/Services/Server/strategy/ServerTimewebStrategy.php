...
use App\Services\Server\timeweb\TimewebService;
use Exception;
use GuzzleHttp\Exception\GuzzleException;
use Illuminate\Support\Facades\Log;

/**
 * Стратегия для работы с серверами провайдера Timeweb Cloud
 * 
 * Реализует операции создания, настройки и управления серверами через API Timeweb Cloud
 */
class ServerTimewebStrategy extends ServerMainStrategy
{
    private TimewebService $service;

    /**
     * @param TimewebService $service Сервис для работы с API Timeweb Cloud
     */
    public function __construct(TimewebService $service)
    {
        $this->service = $service;
    }

    /**
     * Проверка доступности сервера Timeweb Cloud
     *
    ...
    /**
     * Первоначальное создание сервера у провайдера Timeweb Cloud
     *
    ...
    /**
     * Получение пароля сервера от провайдера Timeweb Cloud
     *
    ...
    public function checkStatus(): void
    {
        ...
        $servers = \App\Models\Server\Server::query()
            ->where('provider', \App\Models\Server\Server::TIMEWEB)
            ->where('server_status', \App\Models\Server\Server::SERVER_CREATED)
            ->with(['location', 'panel'])
            ->get();

        \Illuminate\Support\Facades\Log::info('Checking status for Timeweb Cloud servers', [
            'count' => count($servers),
            'source' => 'server'
        ]);
    ...
    public function delete(Server $server): void
    {
        try {
            ...
            Log::error('Error in ServerTimewebStrategy::delete', [
                'server_id' => $server->id,
                'error' => $e->getMessage()
            ]);
            throw $e;
