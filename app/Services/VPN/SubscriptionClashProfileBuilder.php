 ...
 *
 * DNS (redir-host + respect-rules) и sniffer помогают клиентам (Hiddify/Mihomo) применять DOMAIN-SUFFIX
 * к HTTPS по SNI. При зоне «ru» в БД добавляется GEOIP,RU,DIRECT на случай соединений по IP без домена.
 */
class SubscriptionClashProfileBuilder
{
    ...
    /**
     * Зона «весь .ru» в админке (после normalizeDomain) хранится как «ru».
     *
     * @param  array<int, string>  $directDomains
     */
    ...
    {
        foreach ($directDomains as $d) {
            $n = VpnDirectDomain::normalizeDomain((string) $d);
            if ($n === 'ru') {
                return true;
            }
        }
